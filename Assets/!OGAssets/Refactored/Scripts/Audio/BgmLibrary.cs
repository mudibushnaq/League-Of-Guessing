...
public sealed class BgmLibrary : ScriptableObject
{
    ...
    Dictionary<BgmId, CompiledBank> _map;

    void OnEnable()
    {
        _map = new Dictionary<BgmId, CompiledBank>(banks.Count);
    ...
    public bool TryPick(BgmId id, out Variant v)
    {
        v = default;
    ...
    public bool Has(BgmId id) => _map != null && _map.ContainsKey(id);
}